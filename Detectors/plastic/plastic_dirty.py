...
class DirtyPlasticGlovesDetector(Detector):
    ...
    def detect(self):
        ...
        # Apply Gaussian blur to reduce noise and enhance dirty gloves detection
        blurred = cv2.GaussianBlur(grey, (11, 11), 0)

        # Threshold the blurred image to create a binary mask for dirty gloves
        _, thresh = cv2.threshold(blurred, 30, 255, cv2.THRESH_BINARY_INV)

        # Find contours in the binary image
        contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # Draw circles around the detected dirty gloves image
        ...
        # Add text based on dirty gloves detection
        if len(contours) > 0:
        ...
        cv2.imshow("Dirty Gloves Detection Result", result_img)
        cv2.waitKey(0)
